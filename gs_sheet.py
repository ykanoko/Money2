...
S_PERSON1_COLUMN = 10
# S_PERSON2_COLUMN = 11
S_PAY_NAME_COLUMN = 12

# S_PAY_MONEY_COLUMN = 13
...
def money_gs_sheet(t, m, n, p):
    try:
        ...
        s_person2 = 0 - s_person1
        s_pay_name = str(ws.cell(i - 1, S_PAY_NAME_COLUMN).value)
        s_pay_money = abs(s_person1)
        # s_pay_money = float(ws.cell(i - 1, S_MONEY_PAY_COLUMN).value)
        ...
        if t == '収入':
            ...
            if s_person1 < 0:
                ...
                s_pay_money = s_person2
            else:
                s_pay_name = PERSON2_NAME
                s_pay_money = s_person1
        ...
        ws.update_cell(i, S_PAY_NAME_COLUMN, str(s_pay_name))
        # ws.update_cell(i, S_MONEY_PAY_COLUMN, str(s_pay_money))

        return ('[残金]\n'
                'No. ' + str(i - NUMBER_START_ROW) + '\n' + '・' + str(PERSON1_NAME) + 'の残金：' +
                str(money_person1) + '円\n' + '・' + str(PERSON2_NAME) + 'の残金：' + str(money_person2) +
                '円\n'
                '[月支出]\n'
                '・合計：' + str(m_money2) + '円\n' + '・1人当たり：' + str(m_money) + '円\n' + '[精算]\n'
                '・和也：' + str(s_person1) + '円\n' + '・花乃香：' + str(s_person2) + '円\n' + '・払うべき人：' +
...
def monthly_gs_sheet():
    ...
    try:
        ...
        while not ws.cell(j, NUMBER_COLUMN).value == None:
            if '2022/08' in ws.cell(j, DATE_COLUMN).value and ws.cell(j,
                                                                      TYPE_COLUMN).value == '合計支出':
